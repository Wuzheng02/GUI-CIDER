...
def parse_action_type(s):
    ...
    if s.startswith("OPEN_APP"):
        return "OPEN_APP"
    if s.startswith("PRESS_BACK") or (s.startswith("PRESSES_BACK")):
        return "PRESS_BACK"
    if (s.startswith("PRESS_HOME")) or (s.startswith("PRESSES_HOME")):
        return "PRESS_HOME"
    if (s.startswith("PRESS_RECENT")) or (s.startswith("PRESSES_RECENT")):
        return "PRESS_RECENT"
...
def normalize_scroll(s: str) -> str:
    """
    Convert SCROLL action with coordinate format to standard direction format.
    Examples:
        "SCROLL [400, 800, 400, 200]"  -> "SCROLL [UP]"
        "SCROLL [400, 800] , [400, 200]" -> "SCROLL [UP]"
        "SCROLL [100, 200, 300, 200]" -> "SCROLL [RIGHT]"
    If not a SCROLL action or cannot extract four coordinates, return original string.
    """
    s_stripped = s.strip()
    if not s_stripped.startswith("SCROLL"):
        return s_stripped

    # Extract all numbers (including decimals and negative)
    numbers = re.findall(r"-?\d+(?:\.\d+)?", s_stripped)
    if len(numbers) >= 4:
        try:
            ...
            pass
    # Could not parse as coordinate format; return original
    return s_stripped
...
def single_eval(action, label):
    ...
    if type_a == "SCROLL":
        # Normalize both action and label to direction format
        norm_action = normalize_scroll(action)
        norm_label = normalize_scroll(label)
        if norm_action.strip() == norm_label.strip():
            return 1, 1
        else:
            return 1, 0
